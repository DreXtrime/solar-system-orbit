...
export function initUI(planets, callbacks) {
    ...
    document.getElementById('edit-name').addEventListener('input', () => {
        const hasName = document.getElementById('edit-name').value.trim().length > 0
        document.getElementById('extra-planet-fields').classList.toggle('hidden', !hasName)
    })
    // Moon Edit View
    ...
    const moonInputs = ['moon-edit-name', 'moon-edit-size', 'moon-edit-color', 'moon-edit-distance', 'moon-edit-speed']
    moonInputs.forEach(id => {
        document.getElementById(id).addEventListener('input', () => {
            callbacks.onMoonSave(getMoonFormValues(), currentMoon, currentMoonPlanet)
        })
    })
...
export function renderMoonList(planet, onSelect) {
    ...
    document.getElementById('moon-section').classList.toggle('hidden', planet.isStar)
}
...
export function showMoonEditView(moon, planet) {
    ...
    document.getElementById('extra-moon-fields').classList.toggle('hidden', isNew)
    document.getElementById('edit-view').classList.add('hidden')
    document.getElementById('moon-edit-view').classList.remove('hidden')
    document.getElementById('moon-edit-title').textContent = isNew ? 'Add Moon' : 'Edit Moon'
    document.getElementById('btn-moon-delete').classList.toggle('hidden', isNew)
    ...
    document.getElementById('moon-edit-color').value = isNew ? '#ffffff' : '#' + moon.color.toString(16).padStart(6, '0')
    document.getElementById('moon-edit-distance').value = isNew ? 2 : moon.orbitRadius
    document.getElementById('moon-edit-speed').value = isNew ? 0.5 : moon.orbitSpeed
}
...
export function showEditView(planet) {
    ...
    document.getElementById('extra-planet-fields').classList.toggle('hidden', isNew)
...
export function getMoonFormValues() {
    return {
        ...
        color: parseInt(document.getElementById('moon-edit-color').value.replace('#', ''), 16),
        orbitRadius: parseFloat(document.getElementById('moon-edit-distance').value),
        orbitSpeed: parseFloat(document.getElementById('moon-edit-speed').value),
    }
}